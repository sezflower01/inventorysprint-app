-- READ-ONLY PREVIEW, v2. Creates nothing, changes nothing, loads nothing.
--
...
BEGIN
  ...
  SELECT e.asin,
         ...
         round(COALESCE(a.avg_12m, a.avg_all), 2) AS proposed,
         CASE WHEN a.avg_12m IS NOT NULL THEN 'last_12m' WHEN a.avg_all IS NOT NULL THEN 'all_time' END AS basis,
         ...
         round(COALESCE(COALESCE(a.avg_12m, a.avg_all) * s.qty_2026, 0), 2) AS cogs_seeded,
         concat_ws(' ',
           CASE WHEN a.asin IS NULL THEN 'nothing_to_seed' END,
           CASE WHEN a.lots_used = 1 THEN 'one_lot' END,
           CASE WHEN COALESCE(a.avg_12m, a.avg_all) > 0
                 AND abs(lt.last_unit - COALESCE(a.avg_12m, a.avg_all)) / COALESCE(a.avg_12m, a.avg_all) > 0.25 THEN 'drift' END,
           CASE WHEN a.asin IS NOT NULL AND COALESCE(s.qty_2026, 0) > a.units_all THEN 'sold_gt_bought' END,
           CASE WHEN s.cur_unit > 0 AND COALESCE(a.avg_12m, a.avg_all) > 0
                 AND abs(s.cur_unit - COALESCE(a.avg_12m, a.avg_all)) / s.cur_unit > 0.30 THEN 'differs_from_sales' END
         ) AS flags
  FROM excl e
