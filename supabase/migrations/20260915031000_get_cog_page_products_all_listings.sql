-- FIX to 20260915030000: list every valid listing, not only in-stock ones.
--
-- That migration built the page's product list from active_created_listings,
-- the app's "active listing" view. Its own verification showed the view is the
-- wrong definition here: only 856 ASINs came through, against the ~4,550
-- promised to the seller. Beyond excluding FAILED_VALIDATION, the view drops
-- any listing whose inventory row is_ghost_inventory_row -- which includes
-- sold-out products. A sold-out product still has a real COG, and a listing
-- saved without stock is exactly the one the seller needs to cost. With the
-- view, 2,267 existing COG products lost their date and fell to the bottom,
-- and only 30 of the 1,463 uncosted products appeared.
--
-- Now: created_listings filtered by is_active_created_listing(validation_status)
-- -- the view's own status test -- with no stock-based ghost filter.
--
-- The carry-over verification below is re-run unchanged.
...
-- ---- DETAILS ------------------------------------------------------------
--
-- * A listing counts unless it failed validation (is_active_created_listing).
--   Stock is NOT a filter: sold-out products keep their COG and their date.
